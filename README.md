...
安卓或 iPhone 浏览器可直接打开同一个 HTTPS Web 地址；分享 `/?start=register` 会自动进入注册模式并定位到可提交的表单。登录后的手机工作台保留轻量九宫格，直接显示总览与寒冰域、极光域、烈火域、未来雷达、溯源透镜、造界、共振、八度空间；“总览/产品罗盘”完整展示包括光子魅影在内的九个正式产品，避免继续增加固定 Dock 高度。
