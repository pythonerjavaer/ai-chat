...
- 八度空间（Music Dimension）：提供四种可继续改写的创作模板、作品/风格/情绪/BPM/声音材质设定和创作描述蓝图；浏览器可用 Web Audio API 本地合成原创程序化氛围声场，模板改写与声音合成都不调用 OpenAI、消耗 0 Token。创作蓝图、音量与播放偏好保存在当前设备，刷新后仍可继续；Apple Music 仅作为可选授权播放入口，未配置开发者凭证时明确降级。
...
- 未来雷达：岗位/职能、八类雇主星域、行业与城市均为真实筛选条件；同一字段内取并集，不同已填写字段之间同时生效，星域选项点击后会自动重新匹配。结果按个人职业价值模型分为 T0、T0.5、T1、T1.5、T2、T2.5、T3 七档，并可点击层级只查看该档、展开岗位查看主要加减分理由。分层是匹配优先级，不是录取概率：T0 ≥90，T0.5 为 85–89，T1 为 80–84，T1.5 为 75–79，T2 为 70–74，T2.5 为 65–69，T3 为 60–64；低于 60 分不进入重点池。
...
- 可选 OpenAI 网页搜索补漏：设置 `RECRUITMENT_WEB_SEARCH_ENABLED=true` 后，服务按 `RECRUITMENT_WEB_SEARCH_INTERVAL_MINUTES`（默认 360 分钟）搜索八类重点雇主的当前校园岗位，限制网页搜索工具调用次数，过滤非目标单位、非当前目标届别、社招、当天截止/过期/尚未开放条目、搜索结果页和社交媒体链接；模型提交的日期只有在官方页出现同一日期时才会采用。每次调用记录工具次数和实际 Token，该能力会产生 OpenAI API 费用。
...
- 八度空间当前生成的是程序化氛围声场与可复用创作描述，不是完整商业歌曲生成器，也不会模仿具体艺人或提供未授权音乐；Apple Music 播放需要另行配置合法 MusicKit 凭证与用户授权；
...
安卓或 iPhone 浏览器可直接打开同一个 HTTPS Web 地址；分享 `/?start=register` 会自动进入注册模式并定位到可提交的表单。登录后的手机工作台以九宫格直接显示总览与寒冰域、极光域、烈火域、未来雷达、溯源透镜、造界、共振、八度空间八个正式产品入口，不再通过“更多”菜单隐藏产品。
