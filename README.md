...
- AI Space Token 账本：只统计成果胶囊的模型实际输入/输出 Token，并用账号级 Space 权益与每个 Space 的预算在服务端阻止超额调用；服务器规则整理和缓存命中的实际模型用量为 0。普通聊天、文档 Embedding 和冰火交叉审查目前不计入这本账，不能把这里的余额理解为整个应用的 OpenAI 总额度。
- 普通聊天的完整记录保存在数据库，但每次只把最近 16 条消息重新发送给模型，并把每轮最大输出限制为 700 Tokens；冰火交叉审查最大输出为 1,800 Tokens。这样控制成本，也意味着更早的聊天内容不会自动进入当前模型上下文。
- 未来雷达：岗位/职能类型、雇主类型、行业与城市偏好均可只填写其中一部分后匹配，结果按 T0、T0.5、T1、T1.5、T2、T2.5、T3 七档排序。分层是匹配优先级，不是录取概率：T0 ≥90，T0.5 为 85–89，T1 为 78–84，T1.5 为 72–77，T2 为 64–71，T2.5 为 56–63，T3 ≤55。
- 已建立八组互不重叠的监控范围：央企能源与资源、央企科技通信与交通、政策性金融与国有大行、券商基金资管、保险综合金融、互联网大中厂、快消外企咨询、量化私募。它们是高信号搜索范围，不代表每家单位当前均有开放岗位；低优先级金字塔底层不进入默认池。
- 动态源适配器：服务启动后按 `RECRUITMENT_REFRESH_MINUTES`（默认 30 分钟）扫描国聘网、国资小新、银行招聘网等公开招聘页面，并支持配置 Adzuna API 凭证。公开来源会先做校招标题、重点雇主和城市规则过滤，再标记“待打开核对”，不会冒充已核验公告。
- 可选 OpenAI 网页搜索补漏：设置 `RECRUITMENT_WEB_SEARCH_ENABLED=true` 后，服务按 `RECRUITMENT_WEB_SEARCH_INTERVAL_MINUTES`（默认 360 分钟）搜索八类重点雇主的当前校园岗位，限制网页搜索工具调用次数，过滤非目标单位、社招、过期条目、搜索结果页和社交媒体链接，并对每次调用记录工具次数和实际 Token。该能力会产生 OpenAI API 费用。
- 行动卡片准入：只展示带可点击 HTTPS 链接的高信号校招岗位，或通过受保护动态接收 API 写入的岗位；无链接、城市招聘导航页、非校招和已过期岗位不会展示。公开源无法确认统一截止日期时显示为开放岗位，但不会进入截止预警。
- 已核验校招直达源：拼多多 2027 提前批与科尔尼大中华区 2027 校招；远期岗位不进入默认池，也不会占用截止预警。
