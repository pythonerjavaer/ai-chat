...
- 企业分组浏览：机会池前端默认进入“重点机会”并按企业展示，保留 T0–T3 与未评分线索；“全部记录”和“次级机会”可查看其余记录，不删除低分岗位或改动评分。提供“按企业／按岗位”切换，后端先对全池应用重点范围、星域、T 级、城市、来源及搜索条件，再统计和分页；企业页默认每页 20 组，展开后独立分页查看匹配岗位。侧栏分别标明当前筛选的企业组数和机会条数，不把运营商的分支岗位数当作企业数。运营商品牌归组只用于浏览，不改招聘主体、总部／分支判断、岗位评分或原始链接。API 默认 `view=jobs`、`priority_only=false` 保持兼容，支持 `view=companies`、`company_key`、`total_companies` 和 `total_opportunities`。
