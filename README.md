...
- 未来雷达：岗位/职能类型、雇主类型、行业与城市偏好均可只填写其中一部分后匹配，结果按 T0–T3 排序。
...
- 首页截止预警：登录后直接显示 7 天内到期的已核验校招，无需先打开未来雷达；过期岗位不会从岗位 API 返回。只有原公告明确标注的截止日期才会触发预警，例如九坤当前无统一硬截止日期，因此不会被误报为“快到期”。
...
| `GET/POST` | `/api/recruitment/watches` | 查询或添加个人官网变化雷达 |
