"""Conservative initial Source Registry entries.

No private conversation IDs, cookies, account IDs, or unverified WeChat URLs
belong here.  Sources without a legally reliable public discovery endpoint are
explicitly marked ``discovery_limited`` until an administrator configures one.
"""

from __future__ import annotations

import urllib.parse
from typing import Any


# User-confirmed public article anchors.  These are not private account IDs and
# do not grant access to a WeChat account; they give each public-web discovery
# source a concrete, inspectable starting point instead of relying on its name
# alone.  The order reflects the five accounts supplied by the user.
WECHAT_PUBLIC_ARTICLE_ANCHORS = (
    ("wechat-guoyang-campus", "国央校招", "https://mp.weixin.qq.com/s/2YRjkdejWz-AAOcpoS85WA"),
    ("wechat-iguopin", "国聘", "https://mp.weixin.qq.com/s/uXDQ9RBPBcIRAjZV7S2jgg"),
    ("wechat-sasac-xiaoxin", "国资小新", "https://mp.weixin.qq.com/s/fKmsPl5K33HLDk-kUKOZEw"),
    ("wechat-guoyang-career", "国央求职网", "https://mp.weixin.qq.com/s/CqdDhYs-gP6l1J4ipjWSyQ"),
    ("wechat-bank-recruitment", "银行招聘网", "https://mp.weixin.qq.com/s/1fuJNWMpHT7Oiq0kD3poFA"),
)

WECHAT_SOURCE_NAMES = tuple((source_id, name) for source_id, name, _url in WECHAT_PUBLIC_ARTICLE_ANCHORS)

# These were briefly added as separate discovery sources while investigating a
# China Mobile coverage gap.  They are intentionally retained as disabled
# registry rows on existing deployments, so a restart cannot keep scanning
# them, while prior source history remains intact.
RETIRED_WECHAT_SOURCE_NAMES = (
    ("wechat-china-mobile-recruitment", "中国移动招聘"),
    ("wechat-jiutian-recruitment", "中移九天招聘"),
)

# Public article discovery is deliberately independent from the official ATS
# schedule.  These sources search only public, already-indexed material and
# never authenticate to a personal WeChat account.
WECHAT_DISCOVERY_INTERVAL_MINUTES = 1440

EXISTING_PUBLIC_SOURCES = (
    (
        "public-iguopin-campus",
        "国聘网公开校园频道",
        "https://job.iguopin.com/jobList?campus_nature=&channel=campus&key_word=&nature_cn=",
    ),
    (
        "public-sasac-xiaoxin-existing",
        "国资委招聘公开栏目（国资小新官方来源）",
        "https://www.sasac.gov.cn/n2588035/n2588325/n2588350/index.html",
    ),
    ("public-bank-recruitment", "银行招聘网公开入口", "https://yhks.cn/"),
)

VERIFIED_OFFICIAL_SOURCES = (
    {
        "id": "official-dji-digital-2027",
        "name": "大疆 2027 数字管理构建者计划（官网）",
        "platform": "official_web",
        "company": "大疆创新",
        "source_type": "official_html",
        "url": "https://careers.dji.com/zh-CN/campus/digital-recruitment",
        "priority": 95,
        "trust_level": "verification",
        "interval_minutes": 60,
        "adapter_config": {
            "adapter": "official_html",
            "ai_extract": False,
            "required_markers": ["2027", "数字管理构建者计划"],
            "discover_job_links": True,
            "recruitment_year": 2027,
            "recruitment_type": "campus",
            "program_name": "2027 大疆“拓疆者”校园招聘",
            "job_marker": "数字管理构建者计划",
            "job_title": "数字管理构建者计划",
            "opening_date": "2026-08-07",
            "employer_type": "大型科技企业",
            "industry": "科技",
            "requirements": "面向 2027 届及优秀 2026 届高校毕业生；具体资格以官网为准。",
        },
        "query_config": {"recruitment_year": 2027, "scope": "campus"},
        "region_config": {"timezone": "Asia/Shanghai", "regions": ["中国大陆", "香港"]},
        "status": "pending",
        "verification_status": "verified",
    },
    {
        "id": "official-pdd-campus-2027",
        "name": "拼多多集团 2027 届校园招聘（官网）",
        "platform": "official_web",
        "company": "拼多多集团",
        "source_type": "official_html",
        "url": "https://careers.pddglobalhr.com/campus",
        "priority": 94,
        "trust_level": "verification",
        "interval_minutes": 120,
        "adapter_config": {
            "adapter": "official_html",
            "ai_extract": False,
            "required_markers": ["拼多多集团", "2027届校园招聘"],
            "discover_job_links": True,
            "recruitment_year": 2027,
            "recruitment_type": "campus",
            "program_name": "2027 届校园招聘",
            "employer_type": "互联网企业",
            "industry": "互联网",
            "primary_category": "internet_tech",
            "requirements": "面向 2026 年 9 月至 2027 年 8 月毕业的应届生；具体岗位和资格以官网为准。",
        },
        "query_config": {"recruitment_year": 2027, "scope": "campus"},
        "region_config": {"timezone": "Asia/Shanghai", "regions": ["中国大陆"]},
        "status": "pending",
        "verification_status": "verified",
    },
    {
        "id": "official-honor-campus-2027",
        "name": "荣耀 2027 届校园招聘（官网）",
        "platform": "official_web",
        "company": "荣耀",
        "source_type": "official_html",
        "url": "https://www.honor.com/cn/career/",
        "priority": 93,
        "trust_level": "verification",
        "interval_minutes": 180,
        "adapter_config": {
            "adapter": "official_html",
            "ai_extract": False,
            "required_markers": ["荣耀", "荣耀2027届校园招聘全球启动"],
            "discover_job_links": True,
            "recruitment_year": 2027,
            "recruitment_type": "campus",
            "program_name": "荣耀 2027 届校园招聘",
            "opening_date": "2026-08-18",
            "employer_type": "大型科技企业",
            "industry": "科技",
            "primary_category": "internet_tech",
            "requirements": "具体岗位、地点和申请资格以荣耀招聘官网为准。",
            "configured_jobs": [
                {
                    "external_id": "honor-campus-robot-perception-2027",
                    "job_marker": "机器人感知算法工程师",
                    "job_title": "机器人感知算法工程师",
                    "role_tags": ["algorithm", "robotics", "perception"],
                },
                {
                    "external_id": "honor-campus-foundation-model-2027",
                    "job_marker": "大模型算法工程师",
                    "job_title": "大模型算法工程师",
                    "role_tags": ["ai", "foundation_model", "algorithm"],
                },
                {
                    "external_id": "honor-campus-aigc-video-2027",
                    "job_marker": "AIGC 图像视频生成算法工程师",
                    "job_title": "AIGC 图像视频生成算法工程师",
                    "role_tags": ["ai", "aigc", "computer_vision"],
                },
            ],
        },
        "query_config": {"recruitment_year": 2027, "scope": "campus"},
        "region_config": {"timezone": "Asia/Shanghai", "regions": ["中国大陆", "海外"]},
        "status": "pending",
        "verification_status": "verified",
    },
    {
        "id": "official-china-telecom-campus-2027",
        "name": "中国电信 2027 年度校园招聘（官网）",
        "platform": "official_web",
        "company": "中国电信集团有限公司",
        "source_type": "official_html",
        "url": "https://www.chinatelecom.com.cn/ct/zp/168256.html",
        "priority": 96,
        "trust_level": "verification",
        "interval_minutes": 180,
        "adapter_config": {
            "adapter": "official_html",
            "ai_extract": False,
            "required_markers": ["中国电信集团有限公司", "2027年度校园招聘"],
            "recruitment_year": 2027,
            "recruitment_type": "campus",
            "program_name": "2027 年度校园招聘",
            "emit_program_listing": True,
            "opening_date": "2026-08-24",
            "employer_type": "央企科技通信",
            "industry": "通信运营",
            "primary_category": "state_tech_telecom",
            "requirements": "主要面向 2027 届国内外普通高校毕业生；具体单位、岗位和资格以官网为准。",
        },
        "query_config": {"recruitment_year": 2027, "scope": "campus"},
        "region_config": {"timezone": "Asia/Shanghai", "regions": ["中国大陆"]},
        "status": "pending",
        "verification_status": "verified",
    },
    {
        "id": "official-china-telecom-campus-jobs-2027",
        "name": "中国电信 2027 校园招聘岗位列表（官方 ATS）",
        "platform": "official_ats",
        "company": "中国电信集团有限公司",
        "source_type": "official_api",
        "url": "https://wejob.chinatelecom.com.cn/wt/TELE/mobweb/v8/position/list?recruitType=1&brandCode=1",
        "priority": 95,
        "trust_level": "verification",
        "interval_minutes": 60,
        "adapter_config": {
            "adapter": "official_api", "provider": "china_telecom_campus",
            "recruitment_year": 2027, "recruitment_type": "campus",
            "ai_extract": False, "max_pages": 300, "max_scan_seconds": 600,
            "domain_delay_seconds": 1,
        },
        "query_config": {"recruitment_year": 2027, "scope": "campus"},
        "region_config": {"timezone": "Asia/Shanghai", "regions": ["中国大陆"]},
        "status": "pending", "verification_status": "verified",
    },
    {
        "id": "official-china-unicom-campus-2027",
        "name": "中国联通 2027 校园招聘新苗计划（官方 ATS）",
        "platform": "official_ats",
        "company": "中国联合网络通信集团有限公司",
        "source_type": "official_api",
        "url": "https://zglt.zhaopin.com/home/index.html",
        "priority": 95,
        "trust_level": "verification",
        "interval_minutes": 60,
        "adapter_config": {
            "adapter": "official_api", "provider": "china_unicom_campus",
            "recruitment_year": 2027, "recruitment_type": "campus",
            "program_name": "2027 校园招聘", "emit_program_listing": True,
            "requirements": "中国联通官方新苗计划；具体岗位、职能方向及资格以各单位独立招聘详情为准。",
            "ai_extract": False, "max_pages": 200, "max_scan_seconds": 600,
            "domain_delay_seconds": 1,
        },
        "query_config": {"recruitment_year": 2027, "scope": "campus"},
        "region_config": {"timezone": "Asia/Shanghai", "regions": ["中国大陆"]},
        "status": "pending", "verification_status": "verified",
    },
    {
        "id": "official-china-mobile-campus-notices",
        "name": "中国移动官方招聘公告动态源",
        "platform": "official_web",
        "company": "中国移动通信集团有限公司",
        "source_type": "official_api",
        # Public notice JSON called by the official homepage's initNotice().
        # Existing 2026/social notices must never be relabelled as 2027 campus.
        "url": "https://job.10086.cn/personal/notice/5044401_11161_21137.json",
        "priority": 95,
        "trust_level": "verification",
        "interval_minutes": 60,
        "adapter_config": {
            "adapter": "official_api", "provider": "china_mobile_notices",
            "recruitment_year": 2027, "recruitment_type": "campus",
            "ai_extract": False, "domain_delay_seconds": 1,
        },
        "query_config": {"recruitment_year": 2027, "scope": "campus"},
        "region_config": {"timezone": "Asia/Shanghai", "regions": ["中国大陆"]},
        "status": "pending", "verification_status": "verified",
    },
    {
        "id": "official-haier-campus-2027",
        "name": "海尔集团 2027 届校园招聘（官网）",
        "platform": "official_web",
        "company": "海尔集团",
        "source_type": "official_html",
        "url": "https://maker.haier.net/client/campus/index",
        "priority": 90,
        "trust_level": "verification",
        "interval_minutes": 180,
        "adapter_config": {
            "adapter": "official_html",
            "ai_extract": False,
            "required_markers": ["海尔", "海尔集团2027届校园招聘"],
            "discover_job_links": True,
            "recruitment_year": 2027,
            "recruitment_type": "campus",
            "program_name": "海尔集团 2027 届校园招聘",
            "employer_type": "大型消费科技企业",
            "industry": "智能制造与消费科技",
            "primary_category": "consumer_foreign_consulting",
            "requirements": "面向 2027 届应届毕业生；具体岗位和资格以海尔招聘官网为准。",
        },
        "query_config": {"recruitment_year": 2027, "scope": "campus"},
        "region_config": {"timezone": "Asia/Shanghai", "regions": ["中国大陆", "海外"]},
        "status": "pending",
        "verification_status": "verified",
    },
    {
        "id": "official-xiaomi-campus-2027",
        "name": "小米 2027 届全球校园招聘（官网）",
        "platform": "official_web",
        "company": "小米集团",
        "source_type": "official_html",
        "url": "https://hr.xiaomi.com/campus",
        "priority": 92,
        "trust_level": "verification",
        "interval_minutes": 180,
        "adapter_config": {
            "adapter": "official_html",
            "ai_extract": False,
            "required_markers": ["小米", "小米2027届全球校园招聘"],
            "discover_job_links": True,
            "recruitment_year": 2027,
            "recruitment_type": "campus",
            "program_name": "小米 2027 届全球校园招聘",
            "employer_type": "大型科技企业",
            "industry": "科技与智能硬件",
            "primary_category": "internet_tech",
            "requirements": "具体岗位、地点和申请资格以小米招聘官网为准。",
        },
        "query_config": {"recruitment_year": 2027, "scope": "campus"},
        "region_config": {"timezone": "Asia/Shanghai", "regions": ["中国大陆", "海外"]},
        "status": "pending",
        "verification_status": "verified",
    },
    {
        "id": "official-xiaomi-top-talent-2027",
        "name": "小米顶尖应届生项目（官网）",
        "platform": "official_web",
        "company": "小米集团",
        "source_type": "official_html",
        "url": "https://hr.xiaomi.com/website/top-talent.html",
        "priority": 93,
        "trust_level": "verification",
        "interval_minutes": 180,
        "adapter_config": {
            "adapter": "official_html",
            "ai_extract": False,
            "required_markers": ["顶尖应届生项目", "2024年-2027年"],
            "discover_job_links": True,
            "recruitment_year": 2027,
            "recruitment_type": "campus",
            "program_name": "小米顶尖应届生项目",
            "employer_type": "大型科技企业",
            "industry": "科技与智能硬件",
            "primary_category": "internet_tech",
            "configured_jobs": [
                {
                    "external_id": "xiaomi-top-graduate-talent-2027",
                    "job_marker": "顶尖应届生项目",
                    "job_title": "顶尖应届生项目",
                    "role_tags": ["research", "ai", "top_talent"],
                    "requirements": (
                        "面向 2024 年至 2027 年毕业的全球顶尖高校应届生；"
                        "具体方向和资格以小米招聘官网为准。"
                    ),
                }
            ],
        },
        "query_config": {"recruitment_year": 2027, "scope": "campus"},
        "region_config": {"timezone": "Asia/Shanghai", "regions": ["中国大陆", "海外"]},
        "status": "pending",
        "verification_status": "verified",
    },
    {
        "id": "official-zofund-campus-2027",
        "name": "中欧基金 2027 届校园招聘岗位（官网）",
        "platform": "official_web",
        "company": "中欧基金",
        "source_type": "official_html",
        # The generic campus landing page intermittently stalls at the CDN,
        # while the official vacancy pages remain directly readable.  Use a
        # current, concrete vacancy as the deterministic health anchor.
        "url": "https://zofund.zhiye.com/campusxq?jobId=621132304",
        "priority": 97,
        "trust_level": "verification",
        "interval_minutes": 60,
        "adapter_config": {
            "adapter": "official_html",
            "ai_extract": False,
            "required_markers": ["中欧基金", "27届校招-信用研究"],
            # This anchor is one vacancy, not a complete authoritative list.
            # Never retire the other configured vacancies merely because they
            # are not printed on this individual detail page.
            "snapshot_complete": False,
            "recruitment_year": 2027,
            "recruitment_type": "campus",
            "program_name": "中欧基金 2027 届校园招聘",
            "pool_id": "securities_funds_asset",
            "pool_name": "券商、公募与资管",
            "primary_category": "securities_public_funds_asset_management",
            "employer_type": "券商/公募/资管",
            "industry": "公募基金",
            "requirements": "面向 2027 届应届毕业生；具体资格以中欧基金官方招聘页面为准。",
            "configured_jobs": [
                {
                    "external_id": "zofund-campus-621132304",
                    "job_marker": "27届校招-信用研究",
                    "job_title": "27届校招-信用研究",
                    "application_url": "https://zofund.zhiye.com/campusxq?jobId=621132304",
                    "city": "上海市",
                },
                {
                    "external_id": "zofund-campus-621076397",
                    "job_marker": "27届校招-权益研究",
                    "job_title": "27届校招-权益研究",
                    "application_url": "https://zofund.zhiye.com/campusxq?jobId=621076397",
                    "city": "上海市",
                },
                {
                    "external_id": "zofund-campus-621080967",
                    "job_marker": "27届校招-量化研究",
                    "job_title": "27届校招-量化研究",
                    "application_url": "https://zofund.zhiye.com/campusxq?jobId=621080967",
                    "city": "上海市",
                },
                {
                    "external_id": "zofund-campus-621076417",
                    "job_marker": "27届校招-多资产策略研究",
                    "job_title": "27届校招-多资产策略研究",
                    "application_url": "https://zofund.zhiye.com/campusxq?jobId=621076417",
                    "city": "上海市",
                },
                {
                    "external_id": "zofund-campus-621087801",
                    "job_marker": "27届校招-AI投研方案研究",
                    "job_title": "27届校招-AI投研方案研究",
                    "application_url": "https://zofund.zhiye.com/campusxq?jobId=621087801",
                    "city": "上海市",
                },
                {
                    "external_id": "zofund-campus-621082595",
                    "job_marker": "27届校招-渠道销售（湖南）",
                    "job_title": "27届校招-渠道销售（湖南）",
                    "application_url": "https://zofund.zhiye.com/campusxq?jobId=621082595",
                    "city": "湖南省-长沙市",
                },
                {
                    "external_id": "zofund-campus-621074319",
                    "job_marker": "27届校招-财务管理",
                    "job_title": "27届校招-财务管理",
                    "application_url": "https://zofund.zhiye.com/campusxq?jobId=621074319",
                    "city": "上海市",
                },
                {
                    "external_id": "zofund-campus-621076462",
                    "job_marker": "27届校招-AI应用开发",
                    "job_title": "27届校招-AI应用开发",
                    "application_url": "https://zofund.zhiye.com/campusxq?jobId=621076462",
                    "city": "上海市",
                },
                {
                    "external_id": "zofund-campus-621076388",
                    "job_marker": "27届校招-系统开发",
                    "job_title": "27届校招-系统开发",
                    "application_url": "https://zofund.zhiye.com/campusxq?jobId=621076388",
                    "city": "上海市",
                },
                {
                    "external_id": "zofund-campus-620953573",
                    "job_marker": "27届校招-量化风控",
                    "job_title": "27届校招-量化风控",
                    "application_url": "https://zofund.zhiye.com/campusxq?jobId=620953573",
                    "city": "上海市",
                },
            ],
        },
        "query_config": {"recruitment_year": 2027, "scope": "campus"},
        "region_config": {"timezone": "Asia/Shanghai", "regions": ["中国大陆"]},
        "status": "pending",
        "verification_status": "verified",
    },
    {
        "id": "official-citics-headquarters-campus-2027",
        "name": "中信证券 2027 总部校园招聘岗位（官方 API）",
        "platform": "official_ats",
        "company": "中信证券",
        "source_type": "official_api",
        "url": "https://careers.citics.com/campus/headquarters/",
        "priority": 99,
        "trust_level": "verification",
        "interval_minutes": 60,
        "adapter_config": {
            "adapter": "official_api",
            "provider": "citics_headquarters_campus",
            "recruitment_year": 2027,
            "recruitment_type": "campus",
            "program_name": "中信证券 2027 总部校园招聘",
            "pool_id": "securities_funds_asset",
            "pool_name": "券商、公募与资管",
            "primary_category": "securities_public_funds_asset_management",
            "employer_type": "券商/公募/资管",
            "industry": "证券",
            "ai_extract": False,
            "timeout_seconds": 12,
            "domain_delay_seconds": 0.5,
        },
        "query_config": {"recruitment_year": 2027, "scope": "campus"},
        "region_config": {"timezone": "Asia/Shanghai", "regions": ["中国大陆", "香港"]},
        "status": "pending",
        "verification_status": "verified",
    },
    {
        "id": "official-gf-securities-campus-2027",
        "name": "广发证券 2027 届校园招聘岗位（官方 ATS）",
        "platform": "official_ats",
        "company": "广发证券",
        "source_type": "official_api",
        "url": "https://wecruit.hotjob.cn/SU625527c30dcad4021443cdda/pb/school.html",
        "priority": 97,
        "trust_level": "verification",
        "interval_minutes": 60,
        "adapter_config": {
            "adapter": "official_api",
            "provider": "hotjob_campus",
            "suite_key": "SU625527c30dcad4021443cdda",
            "employer_aliases": ["广发证券股份有限公司"],
            "recruitment_year": 2027,
            "recruitment_type": "campus",
            "program_name": "广发证券 2027 届校园招聘",
            "pool_id": "securities_funds_asset",
            "pool_name": "券商、公募与资管",
            "primary_category": "securities_public_funds_asset_management",
            "employer_type": "券商/公募/资管",
            "industry": "证券",
            "ai_extract": False,
            "max_pages": 20,
            "max_details": 120,
            "max_scan_seconds": 240,
            "domain_delay_seconds": 0.5,
        },
        "query_config": {"recruitment_year": 2027, "scope": "campus"},
        "region_config": {"timezone": "Asia/Shanghai", "regions": ["中国大陆"]},
        "status": "pending",
        "verification_status": "verified",
    },
)


def initial_sources(*, web_search_enabled: bool) -> list[dict[str, Any]]:
    sources: list[dict[str, Any]] = []
    for source_id, name, public_article_url in WECHAT_PUBLIC_ARTICLE_ANCHORS:
        # User paused all built-in WeChat collection on 2026-09-21.
        # Keep provenance and public anchors, but neither enabling AI nor a
        # redeploy may silently restart these accounts.
        discovery_adapter = "discovery_limited"
        sources.append({
            "id": source_id,
            "name": name,
            "platform": "wechat",
            "source_type": "wechat_public",
            "url": public_article_url,
            "domain": "mp.weixin.qq.com",
            "account_name": name,
            "account_id": None,
            "enabled": False,
            "priority": 70,
            "trust_level": "discovery",
            "interval_minutes": WECHAT_DISCOVERY_INTERVAL_MINUTES,
            "adapter_config": {
                "adapter": discovery_adapter,
                # The discovery adapter still uses public Web Search and
                # requires public citations for every returned article/job.
                # This seed URL is a search anchor, never an official job URL.
                "public_article_anchors": [public_article_url],
            },
            "query_config": {"recruitment_year": 2027, "scope": "campus"},
            "region_config": {"timezone": "Asia/Shanghai", "regions": ["中国大陆", "香港"]},
            "status": "disabled",
            "verification_status": "unverified",
        })
    for source_id, name in RETIRED_WECHAT_SOURCE_NAMES:
        sources.append({
            "id": source_id,
            "name": name,
            "platform": "wechat",
            "source_type": "wechat_public",
            "url": None,
            "domain": None,
            "account_name": name,
            "account_id": None,
            "enabled": False,
            "priority": 0,
            "trust_level": "discovery",
            "interval_minutes": WECHAT_DISCOVERY_INTERVAL_MINUTES,
            "adapter_config": {"adapter": "discovery_limited"},
            "query_config": {"recruitment_year": 2027, "scope": "campus"},
            "region_config": {"timezone": "Asia/Shanghai", "regions": ["中国大陆", "香港"]},
            "status": "disabled",
            "verification_status": "unverified",
        })
    for source in VERIFIED_OFFICIAL_SOURCES:
        item = dict(source)
        item["domain"] = urllib.parse.urlsplit(str(item["url"])).hostname
        sources.append(item)
    for source_id, name, url in EXISTING_PUBLIC_SOURCES:
        adapter_config = {"adapter": "official_html", "ai_extract": False}
        if source_id == "public-sasac-xiaoxin-existing":
            adapter_config = {
                "adapter": "public_recruitment_index",
                "discovery_kind": "sasac",
            }
        elif source_id == "public-bank-recruitment":
            adapter_config = {
                "adapter": "public_recruitment_index",
                "discovery_kind": "bank",
            }
        sources.append({
            "id": source_id,
            "name": name,
            "platform": "public_web",
            "source_type": "other_public_source",
            "url": url,
            "domain": urllib.parse.urlsplit(url).hostname,
            "enabled": True,
            "priority": 55,
            "trust_level": "discovery",
            "interval_minutes": 120,
            "adapter_config": adapter_config,
            "query_config": {"recruitment_year": 2027, "scope": "campus"},
            "region_config": {"timezone": "Asia/Shanghai", "regions": ["中国大陆", "香港"]},
            "status": "pending",
            "verification_status": "unverified",
        })
    sources.extend([
        {
            "id": "legacy-recruitment-pipeline",
            "name": "冰焰现有已核验岗位池",
            "platform": "internal",
            "source_type": "manual",
            "enabled": True,
            "priority": 90,
            "trust_level": "verification",
            "interval_minutes": 30,
            "adapter_config": {"adapter": "legacy_database"},
            "status": "pending",
            "verification_status": "verified",
        },
        {
            "id": "legacy-search-discovery",
            "name": "历史搜索与同步候选",
            "platform": "internal",
            "source_type": "manual",
            "enabled": True,
            "priority": 89,
            "trust_level": "discovery",
            "interval_minutes": 30,
            "adapter_config": {
                "adapter": "legacy_database",
                "discovery_only": True,
                "close_confirmations": 1,
            },
            "status": "pending",
            "verification_status": "unverified",
        },
        {
            "id": "openai-public-web-search",
            "name": "OpenAI 公共网页补漏",
            "platform": "openai",
            "source_type": "openai_web_search",
            "enabled": web_search_enabled,
            "priority": 30,
            "trust_level": "discovery",
            "interval_minutes": 360,
            "adapter_config": {"adapter": "openai_web_search"},
            "query_config": {"recruitment_year": 2027, "scope": "campus"},
            "region_config": {"timezone": "Asia/Shanghai", "regions": ["中国大陆", "香港"]},
            "status": "pending" if web_search_enabled else "disabled",
            "verification_status": "unverified",
        },
    ])
    return sources
