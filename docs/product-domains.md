...
| 公版书自动导入 | Project Gutenberg / Standard Ebooks 官方书目发现、版本选择、私有原文件、章节与稳定锚点；CTP 首批古籍仅保存书目与来源链接 | `backend/product_domains/public_library.py`、`frontend/src/product-domains.js` |
...
- 公版书只有用户点击“加入跃迁域”时才从已批准的官方域名下载；原 EPUB 经哈希后压缩保存在用户隔离的 `leap_library_objects` 私有对象表，不写 Git 或本地磁盘。未复核的搜索结果以及 Chinese Text Project 书目禁止自动抓取全文。
