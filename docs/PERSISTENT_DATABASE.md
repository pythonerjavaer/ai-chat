...
本工具用于现有 SQLite **完整数据库**的保全与迁移，不是招聘候选重放器，也不会调用 OpenAI。默认仅在本地创建一致性快照、检查并输出表计数和校验哈希；只有显式 `--apply` 才连接 PostgreSQL。
...
因此，**不要先重启、重新部署、改变套餐或切换数据库，再声称旧数据已经安全保留**。新增迁移脚本、完成 Supabase 连接检查，均不代表生产数据已迁移。
...
4. 保留现有 JWT Secret 和用户密码哈希，不在迁移时同时进行认证体系改造。
