...
登录后的前端使用 `/api/future-radar/opportunities` 同时展示可操作的公开线索和已核验记录，并保留各自的真实核验标记；上面的 `/jobs` 和 `/search-updates` 仍是兼容接口。前端默认“均衡精选”及 `view=companies`：只在 T0–T3 与未评分范围内，按十个星域和星域内企业轮转，每星域最多 60 条、每个展示企业全局最多 6 条。该选择是只读投影，不删除记录、不改变 T 级；“全部重点”可查看完整 T0–T3 与未评分线索，“全部记录”和“次级机会”仍可回看同一机会池中的其余记录。后端 API 默认 `view=jobs`、`priority_only=false`、`balanced_only=false` 不变，前端均衡视图显式发送 `balanced_only=true`。星域、城市、来源和关键词先限定完整基础集合，再应用均衡、重点或 T 级投影，最后计算 `total_companies`、`total_opportunities` 和分页。企业页每页默认 20 组；均衡选择先在完整筛选结果上确定，再用 `company_key` 展开同一批具体岗位，不能为企业重新分配配额或把全部岗位拼回已筛选结果。
