...
test("refresh failure preserves the last successful unified snapshot even after filter changes", async () => {
  ...
  assert.equal(r.requestOptions[queryIndex].timeoutMs, 180_000);
});
...
test("pending cards open the unified detail API and expose safe ordinary source anchors", async () => {
  ...
  assert.ok(index >= 0);
  assert.equal(r.requestOptions[index].timeoutMs, 180_000);
...
test("T selection is immediate, and its result/count comes from the complete compact API pool", async () => {
  ...
  assert.match(r.elements.recruitmentJobs.textContent, /最多等待 180 秒.*继续切换.*旧筛选结果不会作为新结果显示/);
  assert.match(r.elements.recruitmentStatus.textContent, /最多等待 180 秒/);
  assert.equal(r.elements.futureRadarOpportunityCount.textContent, "—");
  assert.equal(r.calls.length, 0);
  await r.flushSelection();
  assert.equal(r.calls.length, 1);
  assert.equal(r.requestOptions[0].timeoutMs, 180_000);
...
test("initial and detail cold reads show a 180-second wait without presenting old or partial results", async () => {
  const r = runtime({ fail: false });
  ...
  await new Promise(setImmediate);
  assert.equal(r.cards().length, 0);
  assert.match(r.elements.recruitmentJobs.textContent, /首次读取或扫描更新后可能较慢.*最多等待 180 秒/);
  assert.doesNotMatch(r.elements.recruitmentJobs.textContent, /legacy-/);
  assert.equal(r.state.futureRadar.jobsLoading, true);
  assert.equal(r.requestOptions.at(-1).timeoutMs, 180_000);
  ...
  assert.match(details.textContent, /正在读取完整机会信息.*最多等待 180 秒/);
  assert.equal(r.requestOptions.at(-1).timeoutMs, 180_000);
